function K = stiff_mat_grd_sig_grd_ANS(p,U,q,V,r,W,CP,d,E,nue,ngauss)
        ...
        for kw = 1:ngauss(3)
          ...
          for kv = 1:ngauss(2)
            ...
            for ku = 1:ngauss(1)
              ...
              Ba(:,:,iip) = B_NL_matrix_curv(i,p,u,U,j,q,v,V,k,r,w,W,CP);
              S = get_stress_curv(p,q,r,u,v,w,U,V,W,CP,d,E,nue);
              SG(:,:,iip) = blkdiag(S,S,S);
              iip = iip+1;
            end
          end
          % --------------------------------------
          ...
          for kv = 1:ngauss(2)
            for ku = 1:ngauss(1)
              ...
              ke = Ba(:,:,iip)'*SG(:,:,iip)*Ba(:,:,iip)*gwu*gwv*gww*dJ*map + ke;
              iip = iip+1;
            end
          end
