...
% plotNURBS_solid(p,q,r,U,V,W,CP)
  ...
  [CP,U,V,W] = knot_refine_solid(p,q,r,U,V,W,CP,[],[0.5],Ru);
  % plotNURBS_solid(p,q,r,U,V,W,CP)
  % fixed supports
  rb =[];
  us=[0 1]; vs=[0 1]; ws=[0 0];  dirs=1;
...
%   plot2in1(p,q,r,U,V,W,CP,CPda,rb,fl);
  ...
  enha(iref) = 0.5*da'*fl;
%   enh2 = 0.5*d'*fs
  ene = abs(2*(F*w*L)^2*t^3/E/w/L^3);
%   eneT = abs(2*(F*w*L)^2*t^3/E/w/L^3 + F^2*6*(1+nue)*t/(5*E*w*L));
